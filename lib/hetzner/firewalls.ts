...
function parseRule(v: unknown): HetznerFirewallRule | null {
  ...
  const sourceIps      = Array.isArray(src) ? src.map((x) => s(x).trim()).filter((x) => x.length > 0) : [];
  const destinationIps = Array.isArray(dst) ? dst.map((x) => s(x).trim()).filter((x) => x.length > 0) : [];
  const description = s(v["description"]).trim();
  return { direction, protocol, port, sourceIps, destinationIps, description: description || null };
}
...
// Replaces ALL rules on a firewall (both inbound + outbound)
// Hetzner: POST /firewalls/{id}/actions/set_rules
