"use client";
// app/dashboard/email/EmailClient.tsx
...
export function EmailClient() {
  ...
  async function load() {
    ...
    try {
      const r = await fetch("/api/customer/email", { cache: "no-store" });
      const j = await r.json().catch(() => null) as { ok?: boolean; data?: EmailRow[]; error?: string } | null;
  ...
  const COLS = ["Name", "Resource ID", "Domain Name", "State", "Billing Period", "Created", "Expiry", "Qty", "Payment"];
    ...
    <div className="cy-page-content">
      <div className="cy-dash-wrap">
        ...
        <div style={{ marginBottom: 8 }}>
          <p style={{ fontSize: 11, color: C.faint, letterSpacing: ".05em", margin: "0 0 4px" }}>DASHBOARD / EMAIL &amp; PRODUCTIVITY</p>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, margin: 0 }}>Email &amp; Productivity</h1>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={COLS.length} style={{ padding: "32px 16px", textAlign: "center", fontSize: 13, color: C.faint }}>No email or productivity subscriptions yet.</td></tr>
              ) : (
                rows.map((row, i) => (
                  <tr key={row.subscriptionId}>
                    ...
                    <td style={{ padding: "12px 16px", color: C.text, borderBottom: i < rows.length - 1 ? C.borderB : "none", whiteSpace: "nowrap" }}>{row.domainName ?? "—"}</td>
                    ...
                    <td style={{ padding: "12px 16px", color: C.text, borderBottom: i < rows.length - 1 ? C.borderB : "none" }}>{row.quantity}</td>
