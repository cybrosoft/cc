...
interface RFQ {
  id: string; docNum: string; status: string; title: string | null;
  notes: string | null; issueDate: string; createdAt: string;
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", height: 20, padding: "0 8px", borderRadius: 10, fontSize: 11, fontWeight: 600, background: s.bg, color: s.color, textTransform: "uppercase", letterSpacing: "0.03em", whiteSpace: "nowrap" }}>
      {status.replace(/_/g, " ")}
    </span>
  );
}
...
export function RFQClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setFormError(null);
    try {
      const res  = await fetch("/api/customer/rfq", {
        method: "POST",
        ...
        <div className="cy-dash-wrap">
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: 14 }}>
                  ...
                  <input
                    ...
                    placeholder="e.g. Cloud server for production workload"
                    maxLength={200}
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", overflow: "hidden" }}>
            ...
            {loading ? (
              ...
              rfqs.map((rfq, idx) => (
                <div key={rfq.id} className="cy-row"
                  style={{ display: "flex", alignItems: "center", padding: "10px 14px", borderBottom: idx < rfqs.length-1 ? "1px solid #f3f4f6" : "none", transition: "background 0.1s" }}>
                  <div style={{ flex: 3, minWidth: 0, paddingRight: 8 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "#111827", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {rfq.title ?? rfq.docNum}
                    </div>
                    <div style={{ fontSize: 11.5, color: "#9ca3af", marginTop: 2, fontFamily: "monospace" }}>{rfq.docNum}</div>
                  </div>
