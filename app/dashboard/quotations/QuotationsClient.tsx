...
interface Quotation {
  ...
  total: number; subject: string | null; issueDate: string;
...
function isExpired(v: string | null) { return !!v && new Date(v) < new Date(); }

const SC: Record<string, string> = {
  ISSUED: "#185FA5", SENT: "#185FA5", REVISED: "#854F0B",
  ACCEPTED: "#0F6E56", REJECTED: "#991b1b", EXPIRED: "#6b7280",
  CONVERTED: "#6d28d9", VOID: "#6b7280",
};
...
export function QuotationsClient() {
  const [docs, setDocs] = useState<Quotation[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/customer/sales?type=QUOTATION&limit=100")
      .then(r => r.json())
      .then(d => {
        const list: Quotation[] = (d.documents ?? []).filter((q: Quotation) => q.status !== "DRAFT");
    ...
    <>
      ...
      <div className="cy-page-content"><div className="cy-dash-wrap">
        <div style={{ marginBottom: 20 }}>
          <h1 className="cy-page-title" style={{ margin: "0 0 3px" }}>Quotations</h1>
          <p style={{ margin: 0, fontSize: 13, color: "#9ca3af" }}>
            {loading ? "Loading…" : `${docs.length} quotation${docs.length !== 1 ? "s" : ""}`}
          </p>
        </div>
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", overflow: "hidden" }}>
          <div style={{ display: "flex", padding: "9px 16px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
            <span style={{ flex: 3,   fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>Quotation</span>
            <span style={{ flex: 1.2, fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>Date</span>
            <span style={{ flex: 1.2, fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>Valid Until</span>
          ...
          {loading
            ...
            : docs.length === 0
              ? <div style={{ padding: "40px 16px", textAlign: "center", fontSize: 13, color: "#9ca3af" }}>No quotations yet.</div>
              : docs.map((q, idx) => {
                  ...
                  return (
                    <Link key={q.id} href={`/dashboard/sales/${q.id}`} className="cy-lrow"
                      ...
                      <div style={{ flex: 3, minWidth: 0, paddingRight: 12 }}>
                        ...
                        {q.subject && <div style={{ fontSize: 11.5, color: "#9ca3af", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{q.subject}</div>}
                      </div>
                      <span style={{ flex: 1.2, fontSize: 12.5, color: "#6b7280" }}>{fmtDate(q.issueDate)}</span>
                      <span style={{ flex: 1.2, fontSize: 12.5, color: expired ? "#dc2626" : "#6b7280" }}>{fmtDate(q.validUntil)}</span>
                      <span style={{ flex: 1.2, fontSize: 12.5, fontWeight: 500, color: "#111827", textAlign: "right" as const, paddingRight: 16 }}>
                        {q.total > 0 ? fmt(q.total, q.currency) : "—"}
                      </span>
