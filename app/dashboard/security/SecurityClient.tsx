"use client";
// app/dashboard/security/SecurityClient.tsx
...
export function SecurityClient() {
  ...
  async function load() {
    ...
    try {
      const r = await fetch("/api/customer/security", { cache: "no-store" });
      const j = await r.json().catch(() => null) as { ok?: boolean; data?: SecurityRow[]; error?: string } | null;
  ...
  const COLS = ["Product Name", "Resource ID", "Type", "Domain/Resource Name", "State", "Billing Period", "Created", "Expiry", "Payment"];
    ...
    <div className="cy-page-content">
      <div className="cy-dash-wrap">
        ...
        <div style={{ marginBottom: 8 }}>
          <p style={{ fontSize: 11, color: C.faint, letterSpacing: ".05em", margin: "0 0 4px" }}>DASHBOARD / SECURITY</p>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, margin: 0 }}>Security</h1>
        ...
        <div style={{ border: C.border, background: C.bg, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={COLS.length} style={{ padding: "32px 16px", textAlign: "center", fontSize: 13, color: C.faint }}>No security subscriptions yet.</td></tr>
              ) : (
                rows.map((row, i) => (
                  <tr key={row.subscriptionId}>
                    <td style={{ padding: "12px 16px", fontWeight: 700, color: C.text, borderBottom: i < rows.length - 1 ? C.borderB : "none", whiteSpace: "nowrap" }}>{row.name}</td>
                    ...
                    <td style={{ padding: "12px 16px", color: C.text, borderBottom: i < rows.length - 1 ? C.borderB : "none", whiteSpace: "nowrap" }}>{row.type}</td>
                    <td style={{ padding: "12px 16px", color: C.text, borderBottom: i < rows.length - 1 ? C.borderB : "none", whiteSpace: "nowrap" }}>{row.resourceName ?? "—"}</td>
