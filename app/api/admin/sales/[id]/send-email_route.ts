...
// ── Helper: get or generate PDF buffer ───────────────────────────────────────
async function getPdfBuffer(
  ...
  try {
    ...
    try {
      ...
      // Note: pdfKey DB update happens in the pdf route — here we just cache silently
      return buf;
    } finally {
      await browser.close();
    }
    ...
    return null; // send email without attachment if PDF fails
  }
}
