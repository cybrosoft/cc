...
export const GET = await makeListHandler("CREDIT_NOTE");
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!originDocId)   return NextResponse.json({ error: "originDocId required — credit note must link to original invoice" }, { status: 400 });
