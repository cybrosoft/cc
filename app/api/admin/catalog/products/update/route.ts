...
export async function POST(req: Request) {
  ...
  // Validate addon-specific fields when type is addon
  if (body.type === "addon") {
  ...
  if (conflict) {
    return NextResponse.json(
      { ok: false, error: "A product with this key already exists" },
      { status: 409 }
    );
  }
  ...
  const updated = await prisma.product.update({
    ...
    data: {
      ...
      // addon fields — always clear if type is not addon
      addonPricingType: isAddon ? (body.addonPricingType ?? null) : null,
      addonBehavior:    isAddon ? (body.addonBehavior    ?? null) : null,
      applicableTags:   isAddon ? (body.applicableTags   ?? [])   : [],
      // per_unit only
      addonUnitLabel: isAddon && body.addonPricingType === "per_unit"
        ? (body.addonUnitLabel?.trim() || null) : null,
      addonMinUnits:  isAddon && body.addonPricingType === "per_unit"
        ? (body.addonMinUnits  ?? null) : null,
      addonMaxUnits:  isAddon && body.addonPricingType === "per_unit"
        ? (body.addonMaxUnits  ?? null) : null,
      // percentage only
      addonPercentage: isAddon && body.addonPricingType === "percentage"
  ...
  await prisma.auditLog.create({
    data: {
      ...
      metadataJson: JSON.stringify({
        key, name, type: body.type,
        ...(isAddon && {
          addonPricingType: body.addonPricingType,
