// app/api/admin/customers/[id]/statement/pdf/route.ts
// Generates statement PDF for a customer — admin authenticated.
// Reuses the same /print/statement page with HMAC token.
...
export async function GET(
  ...
  try {
    ...
    const token    = generateStatementToken(customerId, from, to);
    const baseUrl  = process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000";
    const params   = new URLSearchParams({ token, userId: customerId, from, to });
    ...
    const label = from && to
      ? `Statement_of_Accounts_${from}_${to}`
