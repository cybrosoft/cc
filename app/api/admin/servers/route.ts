// FILE: app/api/admin/servers/route.ts
...
type EnrichedStatus = "OK" | "NA";

type EnrichedSummary = {
  vendor: Vendor;
  status: EnrichedStatus | null;
...
export async function GET(req: Request) {
  try {
    ...
    const [total, servers] = await Promise.all([
      ...
      prisma.server.findMany({
        ...
        select: {
          ...
          hetznerApiToken: true, // never returned
    ...
    const data: Row[] = await Promise.all(
      servers.map(async (sv): Promise<Row> => {
        const productCategoryKey = sv.subscription?.product?.category?.key ?? null;
        ...
        if (vendor === "HETZNER") {
          ...
          if (!token || !hzId) {
            enriched = emptyEnriched("HETZNER");
          } else {
            try {
          ...
          if (!ocid || !region) {
            ...
            try {
              ...
              enriched = {
                ...
                // ✅ Oracle backups
                backupExists: o.backupBootExists, // keep old column meaningful
                backupBootExists: o.backupBootExists,
                backupBlockExists: o.backupBlockExists,

                snapshotExists: null, // Oracle snapshot always "-"
        ...
        return {
          ...
          subscription: sv.subscription
            ? {
                ...
                billingProvider: sv.subscription.billingProvider,
                currentPeriodEnd: sv.subscription.currentPeriodEnd
                  ? sv.subscription.currentPeriodEnd.toISOString()
                  : null,
                productCategoryKey,
                provisionLocation: sv.subscription.provisionLocation ?? null,
              }
            : null,
    ...
    return NextResponse.json(
      { ok: false, error: status === 401 ? "Unauthorized" : "Request failed" },
      { status }
    );
  }
}