// FILE: app/admin/customers/new/ui/CreateCustomerForm.tsx
"use client";
...
const SAUDI_PROVINCES = [
  ...
  "Aseer Province", "Tabuk Province", "Hail Province", "al-Qassim Province",
  "Jazan Province", "Najran Province", "Al-Bahah Province", "Al-Jawf Province",
] as const;

type ApiOk  = { ok: true;  user: { id: string; email: string; role: string; marketId: string; customerGroupId: string | null; createdAt: string } };
type ApiErr = { ok: false; error: string };

// ── CustomerTagSelector (same component, shared logic) ────────────────────────
...
export default function CreateCustomerForm({ markets, groups }: { markets: MarketOption[]; groups: GroupOption[] }) {
  ...
  const [customerGroupId, setCustomerGroupId] = useState(defaultGroupId);
  const [fullName,        setFullName]        = useState("");
  const [mobile,          setMobile]          = useState("");
  const [accountType,     setAccountType]     = useState<AccountType>("BUSINESS");
  const [country,         setCountry]         = useState("SA");
  const [province,        setProvince]        = useState<string>(SAUDI_PROVINCES[0] ?? "");
  const [provinceStateText, setProvinceStateText] = useState("");
  ...
  useEffect(() => {
    fetch("/api/admin/catalog/tags", { cache: "no-store" })
      ...
      .then((j: { ok: boolean; data?: TagRow[] } | null) => {
        if (j?.ok) setAllTags(j.data ?? []);
        setTagsLoading(false);
      });
  }, []);

  function resetForm() {
    setEmail(""); setFullName(""); setMobile(""); setAccountType("BUSINESS");
    setCountry("SA"); setProvince(SAUDI_PROVINCES[0] ?? ""); setProvinceStateText("");
    setCompanyName(""); setVatTaxId(""); setCommercialRegistrationNumber("");
    setAddressLine1(""); setAddressLine2(""); setDistrict(""); setCity("");
    setSelectedKeys([]);
  }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!emailNorm) return setMsg({ type: "err", text: "Email is required." });
    if (!marketId)  return setMsg({ type: "err", text: "Market is required." });
    ...
    try {
      ...
      const res = await fetch("/api/admin/users", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({
          email: emailNorm, marketId, customerGroupId: customerGroupId || null,
      ...
      if (!data || !res.ok || data.ok === false) {
        return setMsg({ type: "err", text: (data as ApiErr)?.error ?? "Failed to create." });
      }

      // 2. Assign tags if any selected
      if (selectedKeys.length > 0) {
        await fetch(`/api/admin/users/${encodeURIComponent((data as ApiOk).user.id)}/tags`, {
          method: "POST", headers: { "content-type": "application/json" },
          body: JSON.stringify({ tagKeys: selectedKeys }),
        });
      }

      setMsg({ type: "ok", text: `Created customer: ${(data as ApiOk).user.email}` });
      resetForm();
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="md:col-span-2">
            <Field label="Email *">
              <input className={inputCls} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="customer@company.com" autoComplete="off" inputMode="email" />
            </Field>
          </div>
          ...
          <Field label="Customer Group">
            <select className={inputCls} value={customerGroupId} onChange={(e) => setCustomerGroupId(e.target.value)}>
              {groups.map((g) => (
                <option key={g.id} value={g.id}>{g.name} ({g.key}){g.isDefault ? " — Default" : ""}</option>
              ))}
            </select>
          </Field>
        </div>
      ...
      {/* ── Location ── */}
      <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
        <h2 className="text-sm font-semibold text-gray-700">Location &amp; Address</h2>
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Country">
            <input className={inputCls} value={country} onChange={(e) => {
              const next = e.target.value.toUpperCase();
              setCountry(next);
              if (next === "SA") { setProvince(SAUDI_PROVINCES[0] ?? ""); setProvinceStateText(""); }
              else setProvince("");
