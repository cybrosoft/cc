...
import type { CustomerOption, CreateSubResp } from "../subscriptionsTableTypes";
...
type RichProduct = {
  ...
  tags: Tag[]; billingPeriods: string[];
  ...
  addonUnitLabel: string | null; addonMinUnits: number | null;
  addonMaxUnits: number | null; addonPercentage: number | null;
  applicableTags: string[]; unitLabel: string | null;
};
type RichEligibleResp = {
  ok: true; currency: string; customerGroup: string;
  market: { key: string; name: string };
  plans: RichProduct[]; addons: RichProduct[]; services: RichProduct[];
};
type LocationRow = {
...
const C = {
  primary: "#318774", primaryBg: "#eaf4f2", primaryMid: "#a7d9d1",
  border: "#e2e8f0", borderL: "#f1f5f9", text: "#0f172a",
  muted: "#64748b", faint: "#94a3b8", header: "#1a1a2e",
};
...
const LABEL: React.CSSProperties = {
  ...
  textTransform: "uppercase" as const, color: "#94a3b8", marginBottom: 6, display: "block",
};
...
function Section({ title, icon, children, dim }: { title?: string; icon?: string; children: React.ReactNode; dim?: boolean }) {
  return (
    <div style={{ marginBottom: 22, opacity: dim ? 0.35 : 1, pointerEvents: dim ? "none" : undefined, transition: "opacity 0.2s" }}>
      {title && (
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 12, paddingBottom: 8, borderBottom: "1px solid #f1f5f9" }}>
          {icon && <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, background: "#eaf4f2", flexShrink: 0 }}><Icon name={icon} size={13} color="#318774" /></span>}
...
function SearchableSelect({ options, value, onChange, placeholder, renderItem, keyExtractor, labelExtractor, disabled }: {
  options: unknown[]; value: string; onChange: (v: string) => void;
  placeholder: string; renderItem: (item: unknown) => React.ReactNode;
  keyExtractor: (item: unknown) => string; labelExtractor: (item: unknown) => string; disabled?: boolean;
}) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <div style={{ ...INP, cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: disabled ? "#f8fafc" : "#ffffff", color: selectedLabel ? "#0f172a" : "#94a3b8", borderColor: open ? "#318774" : "#e2e8f0", boxShadow: open ? "0 0 0 3px rgba(49,135,116,0.1)" : "none" }}
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 2px)", left: 0, right: 0, zIndex: 100, background: "#fff", border: "1px solid #318774", boxShadow: "0 8px 24px rgba(0,0,0,0.10)", maxHeight: 260, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.length === 0 && <div style={{ padding: "16px 12px", color: C.faint, fontSize: 12, textAlign: "center" }}>No results</div>}
            {filtered.map(o => {
              const k = keyExtractor(o);
              const isSelected = k === value;
              return (
                <div key={k} onClick={() => { onChange(k); setOpen(false); setQuery(""); }}
                  style={{ padding: "8px 12px", cursor: "pointer", fontSize: 13, background: isSelected ? C.primaryBg : "#fff", color: isSelected ? C.primary : C.text, borderBottom: `1px solid ${C.borderL}` }}
...
export function CreateSubscriptionModal({ open, onClose, onCreated }: {
  ...
  const plansAndServices = useMemo(() => {
    const base = [...(eligible?.plans ?? []), ...(eligible?.services ?? []).filter(p => p.type === "service")];
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (isRecord(j) && readBoolean(j, "ok")) setEligible(j as unknown as RichEligibleResp);
        else setMsg(isRecord(j) ? (readString(j, "error") ?? "Failed to load products") : "Failed to load products");
  ...
  useEffect(() => {
    if (selectedPlan) {
      ...
      const sorted = [...selectedPlan.prices].sort((a, b) => PERIOD_ORDER.indexOf(a.billingPeriod) - PERIOD_ORDER.indexOf(b.billingPeriod));
      setBillingPeriod(sorted[0]?.billingPeriod ?? "");
    } else { setBillingPeriod(""); }
    setAddonStates({});
  }, [selectedPlan]);

  useEffect(() => {
    if (filteredLocations.length > 0) { const def = filteredLocations.find(l => l.isDefault) ?? filteredLocations[0]; setLocationCode(def.code); }
    else setLocationCode("");
  }, [filteredLocations]);

  useEffect(() => {
    if (filteredTemplates.length > 0) { const def = filteredTemplates.find(t => t.isDefault) ?? filteredTemplates[0]; setTemplateSlug(def.slug); }
  ...
  useEffect(() => {
    setAddonStates(prev => {
      ...
      for (const a of visibleAddons) next[a.id] = prev[a.id] ?? { selected: a.addonBehavior === "required", quantity: a.addonMinUnits ?? 1 };
      return next;
    });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(15,23,42,0.6)", backdropFilter: "blur(2px)", display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "20px 16px" }}>
      <div style={{ width: "100%", maxWidth: 820, background: "#fff", boxShadow: "0 24px 64px rgba(0,0,0,0.2)", marginBottom: 20, borderTop: "3px solid #318774" }}>
        ...
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "15px 22px", background: "#1a1a2e", borderBottom: "1px solid #2d2d44" }}>
          <div>
          ...
          <button onClick={onClose} style={{ background: "none", border: "1px solid #3d3d5c", color: "#94a3b8", cursor: "pointer", padding: "5px 12px", fontSize: 12, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 260px", minHeight: 500 }}>
          ...
          <div style={{ padding: "24px 26px", borderRight: "1px solid #f1f5f9", overflowY: "auto", maxHeight: "80vh" }}>
            {loadingForm && (
              <div style={{ textAlign: "center", padding: 48, color: "#94a3b8", fontSize: 13, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                <Icon name="loader" size={20} color="#a7d9d1" />Loading form…
              </div>
            )}
            {!loadingForm && (<>
              ...
              <Section title="Select Customer" icon="user">
                <SearchableSelect options={customers} value={custId} onChange={setCustId} placeholder="Search by email…"
                  keyExtractor={o => (o as CustomerOption).id} labelExtractor={o => (o as CustomerOption).email}
                ...
                {custId && loadingProducts && <div style={{ fontSize: 11, color: C.faint, marginTop: 6 }}>Loading products…</div>}
              </Section>

              {/* 2. Filter by Location */}
              {custId && (
                <Section title="Filter by Location" icon="pin">
                  <select value={filterLocId} onChange={e => setFilterLocId(e.target.value)} style={{ ...INP }}>
                    <option value="">🌐 All Locations</option>
                    {locationGroups.map(g => (
                      <optgroup key={g.family} label={g.family}>
                        {g.locs.map(l => <option key={l.id} value={l.id}>{l.flag ? `${l.flag} ` : ""}{l.name} ({l.code})</option>)}
                      </optgroup>
                    ))}
                    {standaloneLocations.map(l => <option key={l.id} value={l.id}>{l.flag ? `${l.flag} ` : ""}{l.name} ({l.code})</option>)}
                  </select>
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 5 }}>Filters plans &amp; services to those available at this location</div>
                </Section>
              )}
              ...
              <Section title="Plan or Service" icon="box" dim={!custId || loadingProducts}>
                {eligible && plansAndServices.length === 0 ? (
                  <div style={{ fontSize: 12, color: "#dc2626", padding: "10px 12px", background: "#fef2f2", border: "1px solid #fca5a5" }}>
                    {filterLocId ? "No plans or services available at this location — try \"All Locations\"." : "No priced products available for this customer's market and group."}
                  </div>
                ) : eligible && (
                  <>
                    <SearchableSelect options={plansAndServices} value={planId} onChange={setPlanId}
                      ...
                      renderItem={o => {
                        ...
                        return (
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", flexShrink: 0, background: p.type === "plan" ? "#eff6ff" : "#fffbeb", color: p.type === "plan" ? "#1d4ed8" : "#b45309", border: `1px solid ${p.type === "plan" ? "#bfdbfe" : "#fde68a"}` }}>{p.type.toUpperCase()}</span>
                            <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {selectedPlan && (
                      <div style={{ marginTop: 8, padding: "9px 13px", background: "#eaf4f2", border: "1px solid #a7d9d1", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        ...
                        {selectedPlan.tags.map(t => <span key={t.key} style={{ fontSize: 9, padding: "1px 6px", background: "#fff", color: C.muted, border: `1px solid ${C.border}` }}>{t.name}</span>)}
                        {selectedPlan.prices[0]?.isOverride && <span style={{ fontSize: 9, padding: "1px 6px", background: "#f5f3ff", color: "#7c3aed", border: "1px solid #ddd6fe", marginLeft: "auto" }}>★ Enterprise price</span>}
                      </div>
                    )}
                    {selectedPlan && isPlanMetered && (
                      <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "#fffbeb", border: "1px solid #fde68a" }}>
                        ...
                        <input type="number" value={planQty} min={1} onChange={e => setPlanQty(Math.max(1, Number(e.target.value)))} style={{ ...INP, width: 80, padding: "4px 8px", marginLeft: "auto" }} />
                      </div>
                    )}
                    {selectedPlan && selectedPlan.prices.length > 0 && (
                      <div style={{ marginTop: 14 }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                          {[...selectedPlan.prices].sort((a, b) => ["MONTHLY","SIX_MONTHS","YEARLY","ONE_TIME"].indexOf(a.billingPeriod) - ["MONTHLY","SIX_MONTHS","YEARLY","ONE_TIME"].indexOf(b.billingPeriod)).map(price => {
                            ...
                            return (
                              <button key={price.billingPeriod} type="button" onClick={() => setBillingPeriod(price.billingPeriod)}
                                style={{ padding: "10px 18px", fontFamily: "inherit", cursor: "pointer", background: isActive ? "#318774" : "#ffffff", color: isActive ? "#ffffff" : "#0f172a", border: `2px solid ${isActive ? "#318774" : "#e2e8f0"}`, transition: "all 0.15s", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, minWidth: 90, boxShadow: isActive ? "0 2px 8px rgba(49,135,116,0.25)" : "none" }}>
                                <span style={{ fontSize: 13, fontWeight: 700 }}>{PERIOD_LABELS[price.billingPeriod] ?? price.billingPeriod}</span>
                                <span style={{ fontSize: 11, fontWeight: 500, opacity: isActive ? 0.92 : 0.7 }}>{fmt(price.priceCents, price.currency)}{price.isOverride && <span style={{ marginLeft: 3, fontSize: 9 }}>★</span>}</span>
                              </button>
                            );
                          })}
                        </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: filteredLocations.length > 0 && filteredTemplates.length > 0 ? "1fr 1fr" : "1fr", gap: 12 }}>
                    {filteredLocations.length > 0 && (
                      <div>
                        ...
                        <SearchableSelect options={filteredLocations} value={locationCode} onChange={setLocationCode} placeholder="Select location…"
                          keyExtractor={o => (o as LocationRow).code} labelExtractor={o => { const l = o as LocationRow; return `${l.flag ? l.flag + " " : ""}${l.name}`; }}
                          renderItem={o => { const l = o as LocationRow; return (<div style={{ display: "flex", alignItems: "center", gap: 8 }}>{l.flag && <span style={{ fontSize: 16 }}>{l.flag}</span>}<div><div style={{ fontWeight: 500 }}>{l.name}</div>{l.family && <div style={{ fontSize: 11, color: C.faint }}>{l.family}</div>}</div>{l.isDefault && <span style={{ fontSize: 9, padding: "1px 5px", background: C.primaryBg, color: C.primary, border: "1px solid #a7d9d1", marginLeft: "auto" }}>DEFAULT</span>}</div>); }} />
                      </div>
                    )}
                    {filteredTemplates.length > 0 && (
                      <div>
                        <div style={LABEL}>OS Template</div>
                        <SearchableSelect options={filteredTemplates} value={templateSlug} onChange={setTemplateSlug} placeholder="Select template…"
                          keyExtractor={o => (o as OsTemplateRow).slug} labelExtractor={o => (o as OsTemplateRow).name}
                          renderItem={o => { const t = o as OsTemplateRow; return (<div style={{ display: "flex", alignItems: "center", gap: 8 }}>{t.iconType === "devicon" && t.iconValue && <i className={t.iconValue} style={{ fontSize: 18, color: C.muted }} />}<div><div style={{ fontWeight: 500 }}>{t.name}</div>{t.family && <div style={{ fontSize: 11, color: C.faint }}>{t.family}</div>}</div>{t.isDefault && <span style={{ fontSize: 9, padding: "1px 5px", background: C.primaryBg, color: C.primary, border: "1px solid #a7d9d1", marginLeft: "auto" }}>DEFAULT</span>}</div>); }} />
                      </div>
                    )}
                  </div>
                </Section>
              ...
              <Section title="Price Override" icon="money" dim={!selectedPlan}>
                <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 10 }}>
                  <input type="checkbox" checked={overridePrice} onChange={e => setOverridePrice(e.target.checked)} style={{ width: 15, height: 15, accentColor: C.primary }} />
                ...
                {overridePrice && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <input type="number" value={manualTotal} onChange={e => setManualTotal(e.target.value)} placeholder="0.00" step="0.01" style={{ ...INP, width: 160 }} />
                  </div>
                )}
              </Section>
              ...
              {msg && (
                <div style={{ fontSize: 12, color: "#dc2626", padding: "9px 13px", background: "#fef2f2", border: "1px solid #fca5a5", marginTop: 8, display: "flex", alignItems: "center", gap: 7 }}>
                  <Icon name="alertCircle" size={14} color="#dc2626" />{msg}
                </div>
              )}
            </>)}
          </div>
          ...
          <div style={{ padding: "22px 20px", background: "#f8fafc", display: "flex", flexDirection: "column", gap: 14, borderLeft: "1px solid #f1f5f9" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.1em", paddingBottom: 10, borderBottom: "1px solid #e2e8f0" }}>Order Summary</div>
            {!selectedPlan ? (
              ...
              <div style={{ padding: "10px 0", borderBottom: `1px solid ${C.borderL}` }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: C.primary }}>{planPrice ? fmt(planPrice.priceCents * (isPlanMetered ? planQty : 1), currency) : "—"}</span>
                </div>
              </div>
              ...
              <div style={{ marginTop: 8, padding: "12px 0", borderTop: `2px solid ${C.border}` }}>
                {overridePrice && manualTotal ? (<>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.faint, marginBottom: 4 }}>
                    <span>Calculated</span><span style={{ textDecoration: "line-through" }}>{fmt(computedTotal, currency)}</span>
                  </div>
            ...
            <div style={{ marginTop: "auto" }}>
              <button disabled={busy || !selectedPlan || !billingPeriod || loadingProducts} onClick={() => void handleCreate()}
                style={{ width: "100%", padding: "12px 0", fontSize: 13, fontWeight: 700, fontFamily: "inherit", cursor: busy || !selectedPlan || !billingPeriod ? "not-allowed" : "pointer", background: busy || !selectedPlan || !billingPeriod ? "#e2e8f0" : "#318774", color: busy || !selectedPlan || !billingPeriod ? "#94a3b8" : "#ffffff", border: "none", transition: "background 0.15s", letterSpacing: "0.02em", boxShadow: (!busy && selectedPlan && billingPeriod) ? "0 2px 8px rgba(49,135,116,0.3)" : "none" }}>
                {busy ? "Creating…" : "Create Subscription"}
              </button>
              <button onClick={onClose} style={{ width: "100%", marginTop: 8, padding: "9px 0", fontSize: 13, fontFamily: "inherit", cursor: "pointer", background: "transparent", color: "#64748b", border: "1px solid #e2e8f0" }}>Cancel</button>
            </div>
          </div>
        </div>
      </div>
