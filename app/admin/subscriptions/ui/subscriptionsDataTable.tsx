...
function BillingTab({ sub, addons, currency, renewalData, invoicesData, manualInvoice: initManualInvoice, selectedLineIds: initSelectedLineIds, onChanged }: {
  ...
  const invoicingMode = (sub as any).invoicingMode ?? "AUTO";
  ...
  const pendingAmount = isPartial && grandTotal > 0 ? Math.max(0, grandTotal - paidAmount) : (!isPaid ? grandTotal : 0);
  const pct = (() => {
  ...
  async function uploadReceipt(file: File) {
    ...
    try {
      ...
      setUploadMsg("✓ Uploaded"); onChanged();
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.text, marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          Invoice
          <span style={{ marginLeft: "auto", fontSize: 9, fontWeight: 700, padding: "2px 8px", background: isManualMode ? "#fffbeb" : C.primaryBg, color: isManualMode ? "#92400e" : C.primary, border: `1px solid ${isManualMode ? "#fde68a" : C.primaryMid}` }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        {(() => {
          // If an invoice is linked (auto or manual), pull status from the invoice instead of sub.paymentStatus
          ...
          // No invoice linked — fall back to subscription paymentStatus
          const bg    = isPaid ? "#f0fdf4"  : isPartial ? "#fff7ed"  : "#fef9c3";
          const border= isPaid ? "#86efac"  : isPartial ? "#fdba74"  : "#fde047";
          const color = isPaid ? "#15803d"  : isPartial ? "#c2410c"  : "#92400e";
          ...
          return (
            <div style={{ padding: "10px 14px", background: bg, border: `1px solid ${border}` }}>
              ...
              <div style={{ fontSize: 11, color, marginTop: 4, lineHeight: 1.6 }}>
                ...
                {isPartial && amountNum > 0 && <div>Paid: {amountNum.toFixed(2)} {currency}</div>}
                {isPartial && grandTotal > 0 && <div style={{ fontWeight: 700 }}>Pending: {pendingAmount.toFixed(2)} {currency}</div>}
              </div>
            </div>
          );
        })()}
      ...
      {/* ── Payment record + Payment status — hidden when auto-invoice exists ── */}
          ...
          <div style={{ background: "#f8fafc", border: `1px solid ${C.border}`, padding: "12px 14px" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
              {(["UNPAID","PARTIAL","PAID"] as const).map(ps => (
                <button key={ps} onClick={() => setPayStatus(ps)}
                  style={{ padding: "6px 14px", fontSize: 12, fontWeight: 600, fontFamily: "inherit", cursor: "pointer",
                    background: payStatus === ps ? (ps === "PAID" ? "#f0fdf4" : ps === "PARTIAL" ? "#fff7ed" : "#fef9c3") : "#fff",
                    color:      payStatus === ps ? (ps === "PAID" ? "#15803d" : ps === "PARTIAL" ? "#c2410c" : "#92400e") : C.muted,
                    border:     `2px solid ${payStatus === ps ? (ps === "PAID" ? "#86efac" : ps === "PARTIAL" ? "#fdba74" : "#fde047") : C.border}` }}>
      ...
      {/* ── Receipt — hidden when an invoice is linked (auto or manual) ── */}
      {!hasAutoInvoice && !lookedUpInvoice && !initManualInvoice && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.text, marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" as const }}>
            ...
            {sub.receiptUrl && (
              <>
                ...
                {sub.receiptUploadedAt && <span style={{ fontSize: 11, color: C.faint }}>Uploaded {fmtDate(sub.receiptUploadedAt)}</span>}
              </>
            )}
            {uploadMsg && <span style={{ fontSize: 11, color: uploadMsg.startsWith("✓") ? "#15803d" : "#dc2626" }}>{uploadMsg}</span>}
            {!sub.receiptUrl && !uploadMsg && <span style={{ fontSize: 11, color: C.faint }}>No receipt uploaded — PDF, PNG or JPG</span>}
          </div>
        </div>
      )}
