...
interface LegalInfo {
  companyName?: string; companyNameAr?: string; tagline?: string;
  taxLabel?: string; taxNumber?: string; vatNumber?: string;
  address1?: string; address2?: string; district?: string; city?: string;
  state?: string; postalCode?: string; country?: string; address?: string;
  email?: string; phone?: string; footerText?: string;
  bankDetails?: { bankName?: string; accountName?: string; iban?: string; swift?: string; currency?: string; };
}
interface CompanyProfile { logoUrl?: string; primaryColor?: string }
interface FullDoc {
  id: string; docNum: string; type: string; status: string;
  currency: string; subtotal: number; vatPercent: number; vatAmount: number; total: number;
  ...
  customer: {
    ...
    district?: string | null; city?: string | null; province?: string | null; country?: string | null;
  };
  market: {
    id: string; key: string; name: string; defaultCurrency: string;
    vatPercent: number; legalInfo: LegalInfo | null; companyProfile: CompanyProfile | null;
    showPayOnline: boolean; stripePublicKey?: string | null;
  };
  lines: DocLine[]; payments: Payment[];
...
export default function SalesDocDetailPage() {
  ...
  const loadLogs = useCallback(async () => {
    setLogsLoading(true);
    try {
      const res = await fetch(`/api/admin/sales/${id}/log`);
      const data = await res.json();
  ...
  useEffect(() => {
    if (searchParams?.get("print") === "1" && doc && !loading) setTimeout(() => window.print(), 500);
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      ${isEmbed ? `
        ...
        .cy-mobile-topbar   { display: none !important; }
        .cy-desktop-topbar  { display: none !important; }
        .cy-mobile-spacer   { display: none !important; }
      ` : ""}
    `;
  ...
  async function sendEmailDirect(mode: "default" | "resend") {
    ...
    try {
      const res = await fetch(`/api/admin/sales/${id}/send-email`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode }) });
      const data = await res.json();
  ...
  async function markAsSent() {
    ...
    setSendDropOpen(false); setSendingDirect(true); setSendMsg(null);
    try {
      const res = await fetch(`/api/admin/sales/${id}/send-email`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "mark_as_sent" }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {!isEmbed && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <button onClick={() => window.history.back()} style={{ background: "none", border: "1px solid #d1d5db", cursor: "pointer", fontFamily: "inherit", padding: "5px 10px", color: CLR.muted, fontSize: 13, lineHeight: 1, flexShrink: 0 }}>←</button>
            <h1 style={{ fontSize: 22, fontWeight: 600, letterSpacing: "-0.02em", color: CLR.text, fontFamily: "monospace", margin: 0, lineHeight: 1 }}>{doc.docNum}</h1>
            <DocTypeBadge type={doc.type} />
            <SalesStatusBadge status={doc.status} />
            <div style={{ flex: 1 }} />
            <button onClick={() => router.push(({ RFQ: "/admin/sales/rfq", QUOTATION: "/admin/sales/quotations", PO: "/admin/sales/po", DELIVERY_NOTE: "/admin/sales/delivery-notes", PROFORMA: "/admin/sales/proforma", INVOICE: "/admin/sales/invoices", CREDIT_NOTE: "/admin/sales/returns" }[doc.type] ?? "/admin/sales") + `/${id}`)}
              ...
              <Icon name="edit" size={13} color="#6b7280" /> Edit
            </button>
            {doc.status !== "VOID" && doc.status !== "CONVERTED" && (
              <button onClick={() => setShowConvert(true)} style={{ padding: "7px 14px", fontSize: 12, fontWeight: 600, lineHeight: 1, background: "#fffbeb", color: "#b45309", border: "1px solid #fcd34d", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>Convert ↗</button>
            )}
            <button onClick={() => setShowStatus(true)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", fontSize: 12, fontWeight: 600, lineHeight: 1, background: "#fff", color: CLR.text, border: "1px solid #d1d5db", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>
              <Icon name="chevron" size={13} color="#6b7280" /> Status
            </button>
            {canRecordPayment && (
              ...
              <div ref={sendDropRef} style={{ position: "relative", flexShrink: 0 }}>
                ...
                {sendDropOpen && (
                  <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, zIndex: 200, background: "#fff", border: "1px solid #e5e7eb", boxShadow: "0 4px 16px rgba(0,0,0,0.10)", minWidth: 210 }}>
                    ...
                    {!hasSentEmail && (<><div style={{ height: 1, background: "#f3f4f6" }} /><DropItem icon="check" label="Mark as Sent" hint="Record as sent without emailing" onClick={markAsSent} /></>)}
                    {hasSentEmail  && (<><div style={{ height: 1, background: "#f3f4f6" }} /><DropItem icon="refresh" label="Resend" hint={lastSent || ""} onClick={() => sendEmailDirect("resend")} /></>)}
                  </div>
                )}
              </div>
            )}
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", padding: "44px 52px", fontSize: 13, color: "#111827" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 0 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 0, maxWidth: "35%", wordBreak: "break-word" }}>
              {cp.logoUrl && <img src={cp.logoUrl} alt={li.companyName ?? "Logo"} style={{ maxHeight: 48, maxWidth: 160, objectFit: "contain", marginBottom: 8, display: "block" }} />}
              <div style={{ fontSize: cp.logoUrl ? 15 : 26, fontWeight: 700, color: primaryColor, marginBottom: 2, lineHeight: 1.1, paddingBottom: cp.logoUrl ? 0 : 15 }}>{li.companyName ?? doc.market.name}</div>
              {li.tagline && <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75 }}>{li.tagline}</div>}
              <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75, marginTop: 2 }}>
                {(li.address1 || li.address) && <span>{[li.address1, li.address2, li.district, li.city, li.state, li.postalCode, li.country].filter(Boolean).join(", ") || li.address}</span>}
                {(li.taxNumber || li.vatNumber) && <span style={{ display: "block" }}>{li.taxLabel ?? "VAT"}: {li.taxNumber ?? li.vatNumber}</span>}
                {li.email && <span style={{ display: "block" }}>{li.email}</span>}
              </div>
            </div>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 26, fontWeight: 700, fontFamily: "monospace", color: primaryColor, lineHeight: 1.1 }}>{doc.docNum}</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 40, marginBottom: 18 }}>
            <div style={{ maxWidth: "35%", wordBreak: "break-word" }}>
              ...
              <div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75, marginTop: 3 }}>
                {(doc.customer.addressLine1 || doc.customer.city) && <span style={{ display: "block" }}>{[doc.customer.addressLine1, doc.customer.addressLine2, doc.customer.district, doc.customer.city, doc.customer.province, doc.customer.country].filter(Boolean).join(", ")}</span>}
                {doc.customer.vatTaxId && <span style={{ display: "block" }}>VAT: {doc.customer.vatTaxId}</span>}
                <span style={{ display: "block" }}>{doc.customer.email}</span>
                {doc.customer.mobile && <span style={{ display: "block" }}>{doc.customer.mobile}</span>}
              </div>
            </div>
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "auto auto", gap: "0 28px" }}>
                ...
                {doc.dueDate && <><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75 }}>Due Date</span><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75, textAlign: "right" }}>{fmtDate(doc.dueDate)}</span></>}
                {doc.validUntil && <><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75 }}>Valid Until</span><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75, textAlign: "right" }}>{fmtDate(doc.validUntil)}</span></>}
                {doc.referenceNumber && <><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75 }}>Reference</span><span style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.75, textAlign: "right", fontFamily: "monospace" }}>{doc.referenceNumber}</span></>}
              ...
              {doc.lines.map((l, i) => (
                <tr key={l.id} style={{ borderBottom: "0.5px solid #f3f4f6", background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                  ...
                  <td style={{ padding: "10px 10px", textAlign: "center" }}>
                    {l.billingPeriod ? <span style={{ display: "inline-block", fontSize: 10, padding: "1px 7px", background: "#f3f4f6", border: "0.5px solid #e5e7eb", color: "#6b7280" }}>{PERIOD_LABEL[l.billingPeriod] ?? l.billingPeriod}</span> : "—"}
                  </td>
                  ...
                  <td style={{ padding: "10px 10px", textAlign: "right", fontFamily: "monospace", fontSize: 12, color: Number(l.discount) > 0 ? "#b45309" : "#9ca3af" }}>{Number(l.discount) > 0 ? `${Number(l.discount)}%` : "—"}</td>
                  <td style={{ padding: "10px 10px", textAlign: "right", fontFamily: "monospace", fontSize: 12, fontWeight: 700 }}>{fmtAmount(l.lineTotal, doc.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", margin: "18px 0 24px" }}>
            <div style={{ minWidth: 300 }}>
              <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", borderBottom: "0.5px solid #f3f4f6", fontSize: 12 }}>
                <span style={{ color: "#6b7280" }}>Subtotal</span><span style={{ fontFamily: "monospace", fontWeight: 600 }}>{fmtAmount(doc.subtotal, doc.currency)}</span>
              </div>
              {Number(doc.vatPercent) > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", borderBottom: "0.5px solid #f3f4f6", fontSize: 12 }}>
                  <span style={{ color: "#6b7280" }}>VAT ({Number(doc.vatPercent)}%)</span><span style={{ fontFamily: "monospace", fontWeight: 600 }}>{fmtAmount(doc.vatAmount, doc.currency)}</span>
                </div>
              )}
              <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 10px", borderTop: `2px solid ${primaryColor}`, marginTop: 4, fontSize: 15, fontWeight: 700 }}>
                <span>Total</span><span style={{ color: primaryColor }}>{fmtAmount(doc.total, doc.currency)}</span>
              </div>
              {totalPaid > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", borderBottom: "0.5px solid #f3f4f6", fontSize: 12 }}>
                  <span style={{ color: "#15803d" }}>Paid{doc.payments.length > 0 && <span style={{ fontSize: 10, color: "#9ca3af", marginLeft: 6 }}>{fmtDate(doc.payments[0].paidAt)}</span>}</span>
              ...
              {balanceDue > 0 && totalPaid > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 10px", borderTop: "1px solid #e5e7eb", marginTop: 6, fontSize: 14, fontWeight: 700 }}>
                  <span>Balance Due</span><span style={{ color: "#dc2626", fontFamily: "monospace" }}>{fmtAmount(balanceDue, doc.currency)}</span>
                </div>
              )}
              {doc.market.showPayOnline && doc.market.stripePublicKey && balanceDue > 0 && (
                <div style={{ marginTop: 12, textAlign: "right" }}>
                  <button style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 20px", background: primaryColor, color: "#fff", fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer", fontFamily: "inherit" }}>Pay Online — {fmtAmount(balanceDue, doc.currency)}</button>
          ...
          {/* Bank + ZATCA */}
          {showBank && (
            <div style={{ padding: "12px 16px", background: "#f9fafb", border: "1px solid #e5e7eb", marginTop: 20, marginBottom: 20, display: "grid", gridTemplateColumns: (isSaudi && doc.type === "INVOICE") ? "1fr auto" : "1fr", gap: 24, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Bank Details</div>
              ...
              {isSaudi && doc.type === "INVOICE" && (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em" }}>ZATCA QR</div>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 10 }}>
                    <div style={{ fontSize: 9, color: "#9ca3af", whiteSpace: "nowrap" }}>Scan to verify with ZATCA portal</div>
                    <div style={{ width: 80, height: 80, border: "0.5px solid #e5e7eb", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {doc.zatcaQrCode ? <img src={`data:image/png;base64,${doc.zatcaQrCode}`} alt="ZATCA QR" style={{ width: 72, height: 72 }} /> :
          ...
          {(doc.termsAndConditions || doc.notes) && (
            <div style={{ borderTop: showBank ? "none" : "0.5px solid #e5e7eb", paddingTop: showBank ? 0 : 20, marginBottom: 20 }}>
              {doc.termsAndConditions && <div style={{ marginBottom: 16 }}><div style={{ fontSize: 10, fontWeight: 700, color: "#6b7280", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 8 }}>Terms &amp; Conditions</div><div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{doc.termsAndConditions}</div></div>}
              {doc.notes && <div><div style={{ fontSize: 10, fontWeight: 700, color: "#6b7280", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 8 }}>Notes</div><div style={{ fontSize: 11, color: "#6b7280", lineHeight: 1.7 }}>{doc.notes}</div></div>}
            </div>
          )}

          {doc.internalNote && <div className="no-print" style={{ marginTop: 16, padding: "12px 16px", background: "#fffbeb", border: "1px solid #fcd34d", fontSize: 12, color: "#92400e" }}><strong>Internal note (admin only):</strong> {doc.internalNote}</div>}
          ...
          {(doc.originDoc || (doc.derivedDocs && doc.derivedDocs.length > 0)) && (
            <div className="no-print" style={{ marginTop: 12, padding: "10px 14px", background: "#f9fafb", border: "1px solid #e5e7eb", fontSize: 12 }}>
              {doc.originDoc && <div style={{ marginBottom: 4 }}><span style={{ color: "#6b7280" }}>Origin: </span><span style={{ fontFamily: "monospace", color: primaryColor, fontWeight: 600, cursor: "pointer" }} onClick={() => router.push(`/admin/sales/${doc.originDoc!.id}`)}>{doc.originDoc.docNum}</span><span style={{ color: "#9ca3af", marginLeft: 6 }}>({doc.originDoc.type.replace(/_/g, " ")})</span></div>}
              {doc.derivedDocs && doc.derivedDocs.length > 0 && <div><span style={{ color: "#6b7280" }}>Derived: </span>{doc.derivedDocs.map(d => <span key={d.id} style={{ fontFamily: "monospace", color: primaryColor, fontWeight: 600, marginRight: 10, cursor: "pointer" }} onClick={() => router.push(`/admin/sales/${d.id}`)}>{d.docNum}</span>)}</div>}
            </div>
          )}
          ...
          {doc.rfqFileUrl && (
            <div className="no-print" style={{ marginTop: 12, padding: "10px 14px", background: "#f9fafb", border: "1px solid #e5e7eb", fontSize: 12, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <button onClick={async () => { const res = await fetch(`/api/admin/sales/attachment?key=${encodeURIComponent(doc.rfqFileUrl!)}`); const data = await res.json(); if (data.url) window.open(data.url, "_blank"); }} style={{ background: "none", border: "none", cursor: "pointer", color: CLR.primary, fontWeight: 600, fontSize: 12, padding: 0, fontFamily: "inherit" }}>View / Download ↗</button>
            </div>
          )}
      ...
      {!isEmbed && (
        <div className="no-print" style={{ maxWidth: 860, margin: "0 auto 24px" }}>
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", padding: "16px 20px" }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase" as const, letterSpacing: "0.05em", marginBottom: 14 }}>Status History</p>
            {logsLoading ? <p style={{ fontSize: 12, color: "#9ca3af" }}>Loading…</p>
            : docLogs.length === 0 ? <p style={{ fontSize: 12, color: "#9ca3af" }}>No status changes recorded yet.</p>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" as const }}>
                        {log.field === "status" ? (<>
                          ...
                          {log.oldValue && <><span style={{ fontSize: 11, padding: "1px 7px", background: "#f3f4f6", border: "1px solid #e5e7eb", color: "#6b7280" }}>{log.oldValue.replace(/_/g, " ")}</span><span style={{ fontSize: 11, color: "#9ca3af" }}>→</span></>}
                          ...
                          <span style={{ fontSize: 12, fontWeight: 600, color: "#111827" }}>{log.field} updated{log.oldValue && <span style={{ fontWeight: 400, color: "#6b7280" }}> · was: {log.oldValue}</span>}</span>
                        )}
                      </div>
      ...
      {/* Modals */}
      {showStatus && (
        <StatusChangeModal docId={id} docNum={doc.docNum} docType={doc.type} docStatus={doc.status}
          onClose={() => setShowStatus(false)}
          onChanged={newStatus => { setDoc(prev => prev ? { ...prev, status: newStatus } : prev); loadLogs(); }}
        />
      )}
      {showConvert && (
        <ConvertModal docId={id} docNum={doc.docNum} docType={doc.type}
      ...
      {showPayment && (
        <RecordPaymentModal
          ...
          onSaved={() => { setShowPayment(false); load(); loadLogs(); }}
        />
      )}
      {sendModalMode && (
        <SendEmailModal docId={id} docNum={doc.docNum} docType={doc.type} docStatus={doc.status}
          customerEmail={doc.customer.email} reminderEnabled={doc.reminderEnabled ?? false} reminderCount={doc.reminderCount ?? 0}
          mode={sendModalMode} onClose={() => setSendModalMode(null)}
          onSent={result => {
            setSendMsg({ ok: true, text: "Email sent successfully" });
            setDoc(prev => prev ? { ...prev, emailSentAt: result.emailSentAt, emailSentCount: result.emailSentCount ?? prev.emailSentCount, reminderCount: result.reminderCount ?? prev.reminderCount, reminderEnabled: result.reminderEnabled ?? prev.reminderEnabled } : prev);
          }}
        />
      )}
    </main>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: "#fff", width: "100%", maxWidth: 440, padding: 24 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div><label style={lbl}>AMOUNT ({doc.currency})</label>
