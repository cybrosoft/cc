...
// Public document print page — auth via signed token: /print/sales/[id]?token=xxx
// NOTE: <html><head><body> are provided by app/print/layout.tsx — do NOT add them here.
...
export default async function PrintPage({ params, searchParams }: Props) {
  ...
  if (!token || !verifyPrintToken(id, token)) {
    return (
      <div style={{ fontFamily: "sans-serif", padding: 40, textAlign: "center", color: "#dc2626" }}>
        <h2>Access Denied</h2>
  ...
  // Page title
  const title = `${typeLabel} ${doc.docNum}`;
          ...
          <div style={{ maxWidth: 740, margin: "0 auto", padding: "20px 10px", background: "#fff", height: "100%" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
              <div style={{ minWidth: 280 }}>
                <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", borderBottom: "0.5px solid #f3f4f6", fontSize: 13 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 10px", background: primaryColor, fontSize: 14, fontWeight: 700 }}>
                  <span style={{ color: "#fff" }}>Total</span>
                  <span style={{ color: "#fff", fontFamily: "monospace" }}>{fmtAmount(doc.total, doc.currency)}</span>
                </div>
                {totalPaid > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 10px", borderBottom: "0.5px solid #f3f4f6", fontSize: 13 }}>
                    <span style={{ color: "#15803d" }}>
                      Paid
                      {doc.payments[0]?.paidAt && <span style={{ fontSize: 11, color: "#666666", marginLeft: 6 }}>{fmtDate(doc.payments[0].paidAt.toString())}</span>}
                    </span>
                ...
                {isSaudi && doc.type === "INVOICE" && (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    ...
                    <div style={{ width: 80, height: 80, border: "0.5px solid #e5e7eb", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {doc.zatcaQrCode
                        ? <img src={`data:image/png;base64,${doc.zatcaQrCode}`} alt="ZATCA QR" style={{ width: 76, height: 76 }} />
                        : <span style={{ fontSize: 9, color: "#9ca3af", textAlign: "center" }}>QR pending</span>
                      }
                    </div>
                  </div>
                )}
